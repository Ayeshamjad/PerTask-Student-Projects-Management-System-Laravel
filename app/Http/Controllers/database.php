...
class database extends Controller
{
    ...
    function addTeachersData(Request $request)
    {
        ...
        return $this->index($request->input('email'), $newuser->password = $request->input('password'),
        $request->input('department'),$request->input('qualification'), 0);
        }catch(Exception $e){
    ...
    function studentSubmitProject(Request $request, $studentname, $teachername, $projectname){
        ...
        if($request->hasFile('vdfile')){
            ...
            $file_name = $studentname.'_vd'.'.'.$file->extension();
    ...
    public function index($email, $password, $registration, $section, $check)
    {
       ...
       if($check==1){
        $mailData = [
            ...
            'body' => 'You are successfully registered on PerTask, Your Email Address: '.$email.' 
            and Password: '.$password.'. Your Registration Number is '.$registration.' and Section: '.$section,
        ];
       ...
       elseif($check==0){
        $mailData = [
            'title' => 'Account Registered',
            'body' => 'You are successfully registered on PerTask, Your Email Address: '.$email.' 
            and Password: '.$password.'. Your Department is '.$registration.' and Qualification: '.$section,
        ];
    ...
    function showProjectsTeacher($teachername, $projectname){
        ...
        return view('teacherprojectview', ['collection' => $data])->with('tea
        
        chername', $teachername)->with('projectname', $projectname);
    }
...
echo $dt->format('Y-m-d H:i:s');
    ...
    function showStudents($teachername){
        $data = DB::select("SELECT * FROM `students` WHERE `fullname` NOT IN (SELECT `fullname` FROM `".$teachername."`)");
